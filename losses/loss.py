...
def auto_ingore_related_reduce_sum_frame_batchsize_MSE(y1,y2,axis_fit_degree,index_=2.0):
  ...
  cost = tf.reduce_sum(tf.reduce_mean(tf.reduce_sum(tf.pow(related_loss, index_), 1), 1))
  return cost
