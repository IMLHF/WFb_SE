...
def get_PESQ_STOI_SDR(test_set_tfrecords_dir, ckpt_dir, set_name):
  ...
  '''
  sess, model, g = build_session(ckpt_dir,None,finalizeG=False)
  with g.as_default():
  ...
  while True:
    try:
      ...
      time_save = time.time()
      sys.stdout.flush()
      x_mag, x_theta, y_mag, y_theta, lengths = sess.run([x_batch, Xtheta_batch, y_batch, Ytheta_batch,
                                                          lengths_batch])
      mask, y_mag_est_en = sess.run(
          [model.mask, model.y_mag_estimation],
          feed_dict={
              model.x_mag: np.concatenate([x_mag, y_mag], axis=0),
              model.lengths: np.tile(lengths,2),
          })

      y_mag_est, y_mag_en = np.split(y_mag_est_en, 2, axis=0)
      y_mag = y_mag_en
...
if __name__ == "__main__":
  ...
  # python3 2_test_CC_and_OC_v2.py 0 test_cc 2>&1 | tee  exp/rnn_speech_enhancement/nnet_CXXX_testcc_V2.log
  # python3 2_test_CC_and_OC_v2.py 0 test_oc 2>&1 | tee  exp/rnn_speech_enhancement/nnet_CXXX_testoc_V2.log
